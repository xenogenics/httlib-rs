...
mod test {
    ...
    fn encodes_supported() {
        ...
        size += encoder
            .encode((&5, &vec![-100i32, 100i32]), &mut dst)
            .unwrap();
        size += encoder.encode((&6, &1i64), &mut dst).unwrap();
        size += encoder
            .encode((&7, &vec![-100i64, 100i64]), &mut dst)
            .unwrap();
        ...
        size += encoder
            .encode((&13, &vec![1.0f32, 2.0f32]), &mut dst)
            .unwrap();
        size += encoder.encode((&14, &1.0f64), &mut dst).unwrap();
        size += encoder
            .encode((&15, &vec![1.0f64, 2.0f64]), &mut dst)
            .unwrap();
        size += encoder.encode((&16, &b"foo".to_vec()), &mut dst).unwrap();
        size += encoder
            .encode((&17, EncoderLit::SInt32(&-10)), &mut dst)
            .unwrap();
        size += encoder
            .encode((&18, EncoderLit::SInt32Vec(&vec![-10i32, 10i32])), &mut dst)
            .unwrap();
        size += encoder
            .encode((&19, EncoderLit::SInt64(&-10)), &mut dst)
            .unwrap();
        size += encoder
            .encode((&20, EncoderLit::SInt64Vec(&vec![-10i64, 10i64])), &mut dst)
            .unwrap();
        size += encoder
            .encode((&21, EncoderLit::Fixed32(&10)), &mut dst)
            .unwrap();
        size += encoder
            .encode((&22, EncoderLit::Fixed32Vec(&vec![1u32, 2u32])), &mut dst)
            .unwrap();
        size += encoder
            .encode((&23, EncoderLit::Fixed64(&10)), &mut dst)
            .unwrap();
        size += encoder
            .encode((&24, EncoderLit::Fixed64Vec(&vec![1u64, 2u64])), &mut dst)
            .unwrap();
        size += encoder
            .encode((&25, EncoderLit::SFixed32(&-10)), &mut dst)
            .unwrap();
        size += encoder
            .encode(
                (&26, EncoderLit::SFixed32Vec(&vec![-10i32, 10i32])),
                &mut dst,
            )
            .unwrap();
        size += encoder
            .encode((&27, EncoderLit::SFixed64(&-10)), &mut dst)
            .unwrap();
        size += encoder
            .encode(
                (&28, EncoderLit::SFixed64Vec(&vec![-10i64, 10i64])),
                &mut dst,
            )
            .unwrap();
        assert_eq!(
